...
class PDFGenerator:
    ...
    def generate_bulletin_salaire(self, employe, resultat: Dict, annee: int, mois: int) -> BytesIO:
        """
        Générer bulletin de paie pour le nouveau système Traitement Salaires v3.0
        """
        employe_data = {
            'id': employe.id,
            'nom': employe.nom,
        ...
        salaire_data = {
            'mois': mois,
            'annee': annee,
            'salaire_brut': resultat.get('salaire_cotisable', 0),
            'salaire_net': resultat.get('salaire_net', 0),
            'jours_travailles': resultat.get('jours_travailles', 0),
            'salaire_base': resultat.get('salaire_base', 0),
            'heures_supp': resultat.get('heures_supplementaires', 0),
            'irg': resultat.get('irg', 0),
            'securite_sociale': resultat.get('retenue_securite_sociale', 0),
            'avances': resultat.get('total_avances', 0),
            'credits': resultat.get('retenue_credit', 0)
        }
        
        periode = {'mois': mois, 'annee': annee}
        
        return self.generate_bulletin_paie(employe_data, salaire_data, periode)
    
    def generate_rapport_salaires(self, resultats: List[Dict], annee: int, mois: int, db: Session) -> BytesIO:
        """
        Générer rapport global PDF des salaires
        """
        buffer = BytesIO()
        ...
        titre = Paragraph(
            f"<b>RAPPORT SALAIRES - {mois_nom.upper()} {annee}</b>",
            self.styles['CustomTitle']
        )
        story.append(titre)
        story.append(Spacer(1, 0.5*cm))
        ...
        # Statistiques globales
        total_net = sum(float(r.get('salaire_net', 0)) for r in resultats)
        total_brut = sum(float(r.get('salaire_cotisable', 0)) for r in resultats)
        total_irg = sum(float(r.get('irg', 0)) for r in resultats)
        
        stats_data = [
            ['Nombre d\'employés', str(len(resultats))],
            ['Masse Salariale Brute', f"{total_brut:,.2f} DA".replace(',', ' ')],
        ...
        # Détails par employé
        detail_data = [['Employé', 'J.Trav.', 'Salaire Base', 'Salaire Brut', 'IRG', 'Salaire Net']]
        
        for r in resultats:
            detail_data.append([
                f"{r.get('employe_nom', '')} {r.get('employe_prenom', '')}",
                str(r.get('jours_travailles', 0)),
                f"{float(r.get('salaire_base', 0)):,.0f}".replace(',', ' '),
                f"{float(r.get('salaire_cotisable', 0)):,.0f}".replace(',', ' '),
                f"{float(r.get('irg', 0)):,.0f}".replace(',', ' '),
                f"{float(r.get('salaire_net', 0)):,.0f}".replace(',', ' ')
            ])
        
        detail_table = Table(detail_data, colWidths=[7*cm, 2*cm, 3.5*cm, 3.5*cm, 3*cm, 3.5*cm])
        detail_table.setStyle(TableStyle([
            ('BACKGROUND', (0, 0), (-1, 0), colors.grey),
            ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),
            ('ALIGN', (0, 0), (-1, -1), 'CENTER'),
