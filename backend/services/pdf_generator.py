...
class PDFGenerator:
    ...
    def generate_client_logistics_balance(self, client_data: dict) -> BytesIO:
        ''''''G�n�rer PDF du solde logistique pour un client''''''
        buffer = BytesIO()
        ...
        date_para = Paragraph(f'G�n�r� le {date_str}', self.styles['CustomBody'])
        elements.append(date_para)
        elements.append(Spacer(1, 0.5*cm))
        if client_data['balance']:
            table_data = [['Type', 'Prises', 'Retourn�es', 'Solde']]
            for item in client_data['balance']:
                table_data.append([item['type_name'], str(item['total_prises']), str(item['total_retournees']), str(item['solde'])])
    ...
    def generate_all_clients_logistics_balance(self, clients_data: list) -> BytesIO:
        ''''''G�n�rer PDF global des soldes logistiques de tous les clients''''''
        buffer = BytesIO()
        doc = SimpleDocTemplate(buffer, pagesize=A4, rightMargin=2*cm, leftMargin=2*cm, topMargin=2*cm, bottomMargin=2*cm)
        elements = []
        ...
        if not clients_data:
            ...
            for idx, client in enumerate(clients_data):
                ...
                for item in client['balance']:
                    table_data.append([item['type_name'], str(item['total_prises']), str(item['total_retournees']), str(item['solde'])])
                table = Table(table_data, colWidths=[6*cm, 3*cm, 3*cm, 3*cm])
                table.setStyle(TableStyle([('BACKGROUND', (0, 0), (-1, 0), colors.lightgrey), ('TEXTCOLOR', (0, 0), (-1, 0), colors.black), ('ALIGN', (0, 0), (-1, -1), 'CENTER'), ('FONTNAME', (0, 0), (-1, 0), 'Helvetica-Bold'), ('FONTSIZE', (0, 0), (-1, -1), 9), ('BOTTOMPADDING', (0, 0), (-1, 0), 8), ('GRID', (0, 0), (-1, -1), 0.5, colors.black)]))
                elements.append(table)
    ...
    # === M�thodes Logistique Clients (v2.4.2) ===
"""
